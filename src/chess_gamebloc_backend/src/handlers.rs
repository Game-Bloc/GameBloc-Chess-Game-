...
fn send_app_message(client_principal: ClientPrincipal, msg: AppMessage) {
    ...
    if let Err(e) = send(client_principal, msg.candid_serialize()) {
        print(format!("Could not send message: {}", e));  // I adjusted this line by adding the 'format' and wrap the msg as one arg
    }
}

pub fn on_close( args: OnCloseCallbackArgs ) {
    print(format!("Client {} is disconnected", args.client_principal));
}
